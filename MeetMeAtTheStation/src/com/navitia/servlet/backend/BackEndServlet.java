...
public void updateNextDeparture() {
	...
	public void creatFrodonSacquet(int num) {
		ArrayList<Long> amis = new ArrayList<Long>();
		amis.add((long) -1);
		
		Utilisateur u = new Utilisateur("Sacquet" + num, "Frodon", "userFrodon",  "frodon"+num+"@gmail.com", "sda", amis);
		
		List<Utilisateur> utilisateurs = ofy().load().type(Utilisateur.class).filter("nom ==", "Sacquet" + num).list();
		if (utilisateurs.size() == 0) {
			System.out.println("AJOUT OK: Frodon Sacquet " + num);
			ofy().save().entity(u).now();
		} else {
			System.out.println("DEJA AJOUTE: Frodon Sacquet " + num);
		}
	}
